...
function runTsc(args: string[], cwd: string): { ok: boolean; output: string } {
  const result = Bun.spawnSync([process.execPath, TSC, ...args], { cwd, stdout: 'pipe', stderr: 'pipe' })
  const decoder = new TextDecoder()
  return { ok: result.exitCode === 0, output: `${decoder.decode(result.stdout)}${decoder.decode(result.stderr)}` }
}
