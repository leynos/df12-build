export const meta = {
  name: 'df12-build',
  description:
    'Drive a roadmap to completion with a parallel worker pool: fan out across the independent unblocked frontier (plan + adversarial design review + implement + dual review concurrently in isolated worktrees), serialize only the rebase/squash-merge behind a merge queue, then audit. Re-selects whenever any task completes.',
  whenToUse:
...
// ---------------------------------------------------------------------------
// Configuration (all overridable through the Workflow `args` object).
// ---------------------------------------------------------------------------
const cfg = args || {}
...
function preamble(worktree) {
  ...
  return [
    ...
    `- ${loc}`,
    '- Use `leta` for all codebase exploration and navigation (leta show / refs / grep / files) instead of ad-hoc ripgrep or read-file.',
...
// Prompt builders
// ---------------------------------------------------------------------------
function selectPrompt(processedIds) {
  return [
    preamble(null),
...
function implementPrompt(task, worktree, plan) {
  return [
    ...
    '     - If coderabbit reports a rate limit (or transient 429/5xx), wait with EXPONENTIAL BACKOFF before retrying: 30s, 60s, 120s, 240s, 480s, capped at ~900s, using `sleep`. Retry up to 6 times. If still rate-limited, record this in the execplan and the openIssues, and continue (do not block the work item indefinitely).',
...
function fixPrompt(task, worktree, plan, blocking, round) {
  return [
    ...
    '',
    'Same per-change discipline as implementation: deterministic gate (`make all`, plus markdownlint/nixie for markdown) first and green, THEN `coderabbit review --agent` with exponential backoff on rate limits, then an atomic commit, then update the execplan with what changed and why. Do not introduce scope beyond the blocking items.',
  ].join('\n')
}
...
function implementAddendumPrompt(task, worktree) {
  ...
  return [
    ...
    '  3. Run `coderabbit review --agent` from inside the worktree; address actionable feedback (highest severity first); re-run `make all` to confirm green. On a coderabbit rate limit (429/5xx) back off exponentially (30s, 60s, 120s, 240s, 480s, cap ~900s; up to 6 tries); if still limited, record it in openIssues and continue.',
...
function auditPrompt(task, worktree) {
  const writeClause = DOCUMENT_AUDIT
    ? `Record your findings as a structured markdown file at docs/issues/audit-${task.id}.md (create docs/issues/ if absent), one section per finding with location and a concrete proposed fix. Run \`make markdownlint\` and \`make nixie\` on it, then commit and push that docs-only change to ${BASE}.`
...
async function runTask(task, mergeLock) {
  ...
  // --- Worktree -----------------------------------------------------------
  phase('Worktree')
  const wt = await agent(worktreePrompt(task), { phase: 'Worktree', label: `worktree:${tag}`, schema: WORKTREE_SCHEMA })
  ...
  if (task.isAddendum) {
    phase('Implement')
    const impl = await agent(implementAddendumPrompt(task, worktree), { phase: 'Implement', label: `addendum:${tag}`, schema: IMPL_SCHEMA })
  ...
  for (let round = 1; round <= MAX_DESIGN_ROUNDS; round++) {
    phase('Plan')
    plan = await agent(planPrompt(task, worktree, designVerdict, round), {
      phase: 'Plan',
    ...
    phase('Design Review')
    designVerdict = await agent(designReviewPrompt(task, worktree, plan, round), {
      phase: 'Design Review',
  ...
  // --- Implement ----------------------------------------------------------
  phase('Implement')
  const impl = await agent(implementPrompt(task, worktree, plan), {
    phase: 'Implement',
    label: `implement:${tag}`,
    schema: IMPL_SCHEMA,
  })
  ...
  for (let round = 1; round <= MAX_REVIEW_ROUNDS; round++) {
    const [codeReview, expertReview] = await parallel([
      () => agent(codeReviewPrompt(task, worktree, plan), { phase: 'Code Review', label: `code-review:${tag} r${round}`, schema: REVIEW_SCHEMA }),
      () => agent(expertReviewPrompt(task, worktree, plan), { phase: 'Expert Review', label: `expert-review:${tag} r${round}`, schema: REVIEW_SCHEMA }),
    ])
    ...
    phase('Implement')
    await agent(fixPrompt(task, worktree, plan, blocking, round), { phase: 'Implement', label: `fix:${tag} r${round}` })
  }
...
async function runAudit(task) {
  phase('Audit')
  const audit = await agent(auditPrompt(task, null), { phase: 'Audit', label: `audit:after-${task.id}`, schema: AUDIT_SCHEMA })
  return audit
}
    ...
    decisions: {
      ...
      items: {
        ...
        properties: {
          ...
          lane: { type: 'string', enum: ['addendum', 'step-task', 'reroute', 'dropped'] },
...
function triagePrompt(stepPrefix, proposals) {
  return [
    ...
    `  - When done, run \`make markdownlint\` and \`make nixie\`; fix any issues. Commit the roadmap and any execplan changes (en-GB imperative subject) and push to origin/${BASE} (docs-only).`,
    '',
...
async function runTriage(stepPrefix, proposals) {
  phase('Remediation')
  return await agent(triagePrompt(stepPrefix, proposals), { phase: 'Remediation', label: `triage:${stepPrefix}`, schema: TRIAGE_SCHEMA })
}
