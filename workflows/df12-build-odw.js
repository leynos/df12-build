...
function makePrompts(config) {
  ...
  function integratePrompt2(task, worktree) {
    ...
    return [
      ...
      `  4. Land the squash ENTIRELY inside this worktree. NEVER \`git switch ${BASE2}\` and never touch the control/root worktree or its checked-out ${BASE2}: that switch fails when ${BASE2} is checked out elsewhere, and it pollutes the control worktree (the root of recurring detritus). Step 2 left the task branch rebased on the current origin/${BASE2}; from here, create or force-reset a temp branch there (\`git switch -C integrate-${roadmapIdSlug(task.id)} origin/${BASE2}\` \u2014 the \`-C\` force-resets the branch onto the freshly fetched origin/${BASE2} whether or not it already exists, so this is safe to repeat and safe when a stale integrate-${roadmapIdSlug(task.id)} branch survives a prior aborted run or a host-level resume), squash-merge the task branch onto it (\`git merge --squash <task-branch>\` then \`git commit\` with a clear squash message summarising the task), and push it straight to the integration branch with \`git push origin HEAD:${BASE2}\`. If the push is rejected non-fast-forward (a sibling advanced origin/${BASE2} since step 2), go back to step 2 \u2014 re-fetch and re-rebase the task branch onto the new origin/${BASE2} \u2014 then redo this step: re-running it force-resets the same temp branch onto the new origin/${BASE2} rather than failing because the branch already exists. Retry until it lands.`,
      "",
