...
function integratePrompt(task, worktree) {
  const markStep = task.isAddendum ? `Tick each completed sub-task in ${ROADMAP}: for every id in [${(task.subtasks || []).join(", ")}], change its nested \`- [ ] ${task.id}.<n>.\` to \`- [x] \u2026\`. LEAVE the parent ${task.id} as \`[x]\` (it was already done). Run \`make markdownlint\` and \`make nixie\`; commit the roadmap update (en-GB).` : `Mark the task done in ${ROADMAP}: change its \`- [ ] ${task.id}.\` to \`- [x] ${task.id}.\`. Run \`make markdownlint\` and \`make nixie\`; commit the roadmap update (en-GB).`;
  return [
    preamble(worktree),
