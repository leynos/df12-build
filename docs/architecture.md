...
| Usage-limit faults | A Codex build stage that exhausts its rolling five-hour (or weekly) usage quota classifies as `usage-limit-fault`, matched ahead of the generic infra pattern it also carries. Because the reset window is hours long, `withInfraRetry` deliberately skips the in-window retry rather than burning an attempt; the run halts, no assessment agent is spawned, and remediation triage skips its writes. The `usageLimitFaults` counter lands in the result. | The halt detail directs the operator to relaunch with `resumeMode: "continue"` once the Codex usage limit resets. | The account's quota window and the durable committed ExecPlan (which makes the eventual resume a warm start) are decisive. |
