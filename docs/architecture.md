...
| Code review                   | Review schemas require verdict and blocker fields; blocker arrays are checked regardless of verdict. Each round runs deterministic gates, then the selected host reviewer, then reviewer agents. Dakar is the default; `reviewTool: 'coderabbit'` selects the retained NDJSON adapter. Both adapters normalize into `HostReviewResult`, share retry policy and blocking severities, and emit bounded terminal telemetry. Between-work-item review fails closed; a terminal deferral halts for assessment rather than continuing unreviewed.                                                                                                                                             | Code-review, expert-review, and fallback instructions define what reviewers inspect; agents do not invoke the selected host reviewer themselves.                                                      | Dakar requires `dakar-review` and `pi` on `PATH` plus `OPENAI_API_KEY`; opt-in CodeRabbit mode requires CodeRabbit CLI authentication. Authentication failure halts as `fatal-auth`.                                   |
