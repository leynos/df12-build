...
ODW/Codex launches use a `.workshop` sidecar adjacent to the target project.
The sidecar holds the copied workflow script, ODW runtime configuration,
workflow arguments, and operator notes for the live run. This keeps
long-running workshops recoverable when `.claude/`, `/tmp`, the target project
checkout, or workflow-owned worktrees are cleaned or switched.
...
- Create a task worktree rooted on the current integration branch, then prove
  once per run that the planning and build adapters can actually write inside a
  sibling task worktree (the host verifies a probe file on disk).
...
The workflow enforces this boundary with a host-verified writable-root preflight
(`worktreeWritePreflight`, enabled by default). After the first task worktree
is created, each adapter that must write into task worktrees is asked to write
an exact token file inside the worktree, and the workflow host checks the bytes
on disk before deleting the probe. A failed probe fails the task at stage
`worktree-write` — excluded from partial-branch assessment because it is a
launch or sandbox fault — and every concurrent task shares the memoized
...
Table 1 shows which workflow responsibilities are host-enforced,
prompt-enforced, or dependent on runtime support.
...
| Dry run                       | `DRY_RUN` returns a terminal `dry-run` status before worktree creation in the fresh-task lane, so no branch, worktree, probe write, planning, review, implementation, integration, audit write, or remediation flush occurs. Recovery/continue resume keeps its own dry-run handling over pre-existing worktrees.                                                                                                                                                                                                                                                                                                                                                                       | Fresh-task dry runs dispatch no agents, so no dry-run prompt wording is reached there.                                                                                                                | A fresh-task dry run creates no worktree and needs no write-heavy permissions.                                                                                                                                         |
...
| Tests and deterministic gates | With `hostCommitGates` on (the default), the host re-runs the configured `commitGates` commands against committed `HEAD` at the start of every dual-review round and once per addendum. These runs are serialized pool-wide, with per-command timeouts and secure streamed logs. With `hostGatesBetweenWorkItems` on (the default), the gates also run after each committed work item before the selected host reviewer. With `csCheck` on (the default), CodeScene runs after commit gates and before host review; any red deterministic gate short-circuits to a fix round without spending reviewer-agent tokens. `impl.gatesGreen` is a claim, not the decision.                    | Implementers and fix agents are told to summon `scrutineer` for `make all` and markdown gates, and warned that the host re-runs the gates.                                                            | The gate commands must be runnable by the host process in the task worktree; `commitGateTimeoutSeconds` kills a hung gate.                                                                                             |
| Code review                   | Review schemas require verdict and blocker fields; blocker arrays are checked regardless of verdict. Each round runs deterministic gates, then the selected host reviewer when host review is enabled, then reviewer agents. Dakar is always host-run; in CodeRabbit mode, host review runs only when `coderabbitHostReview` is enabled, while `false` retains the legacy agent-run path. Both adapters normalize into `HostReviewResult`, share retry policy and blocking severities, and emit bounded terminal telemetry. Between-work-item review fails closed; a terminal deferral halts for assessment rather than continuing unreviewed.                                                                                                                                             | Code-review, expert-review, and fallback instructions define what reviewers inspect; host-run modes do not ask agents to invoke the selected host reviewer, while legacy CodeRabbit mode retains its agent-run command.                                                      | Dakar requires `dakar-review` and `pi` on `PATH` plus `OPENAI_API_KEY`; opt-in CodeRabbit mode requires CodeRabbit CLI authentication only when host review is enabled. Authentication failure halts as `fatal-auth`.                                   |
...
| Integration                   | A JavaScript merge lock serializes integration, and success requires `ok`, `rebased`, `squashMerged`, `pushed`, and roadmap evidence (`roadmapMarkedDone`) — all five are pinned in `INTEGRATE_SCHEMA.required` and re-checked by the host gate, so a squash-merged, pushed branch that was never rebased onto BASE (`rebased` false or absent) halts at stage `integrate` instead of counting as done. Addenda whose implementation evidence is complete but whose `ok` field is false become `manual-merge-ready`, never auto-merged.                                                                                                                                                 | The integration agent is told to rebase, gate, squash, push, and avoid the root worktree.                                                                                                             | GitHub branch permissions, non-fast-forward push handling, and sandbox write access to the task worktree are decisive.                                                                                                 |
| Audit and triage              | Audits run only after pushed integrations; triage only deletes pending proposals when `ok` and `pushed` land. Audit and triage inspection worktrees are built with the same verified fetch/base-arg/verify-reset discipline as the task-setup worktree — passing the configured base to `git donkey` (whose no-argument default is always `main`) so a non-`main` base cannot root on the wrong tree, then resetting to `origin/<base>` — so they cannot silently root on a stale local `<base>`.                                                                                                                                                                                       | Audit and triage prompts classify debt into addendum, step-task, reroute, editorial, or dropped lanes.                                                                                                | `documentAudit=false` and `autoMerge=false` change write behaviour; provider-fault halts leave pending proposals unwritten; roadmap edits still need operator review.                                                  |
...
| Fresh restart                 | Returned `processed`, `results`, `halted`, `assessments`, `recovery` (with `unresolved`), `audits`, `remediationTriage`, `pendingProposals`, and enforcement echoes (`commitGates`, `hostGates`, `hostReview`, `workItemBuild`, `stageAttempts`, `faultMetrics`) describe the run outcome.                                                                                                                                                                                                                                                                                                                                                                                              | Operator notes and prompts describe how to recover.                                                                                                                                                   | `origin/<base>`, durable sidecar files, and clean worktree hoovering are the only recovery source of truth.                                                                                                            |
...
- Recovery controls: `resumePartialBranches` (opt-in fresh-run discovery),
  ...
  `adopt-complete` branches through ordinary integration; `continue` dispatches
  deterministically from the committed ExecPlan `Status`), `resumeTaskId`,
...
- Agent routing: `buildAdapter`/`buildModel`, `planAdapter`/`planModel`,
  `reviewAdapter`/`reviewModel`, and `auditAdapter`/`auditModel`/`auditEffort`.
...
integration use the build adapter/model defaults. Planning and review judgement
default to Claude Code with `claude-opus-4-8`: the plan stage uses
`planAdapter` /`planModel`, while design review, code review, expert review,
and addendum fallback review use `reviewAdapter`/`reviewModel`. Audit is
separately right-sized to `claude-sonnet-5` at medium effort.
...
- Remediation triage is mostly de-duplication plus hypothesis routing, so a
  ...
  still select `triageEscalationModel`, which defaults to the same Sol model,
  so every triage task remains on that route unless explicitly overridden.

A sidecar that wants a different assessment or triage route must say so in
`args.json`.

The host review tool defaults to Dakar. It runs `dakarCommand` (default
`dakar-review`) against the committed diff with `--repo-root`, `--base`,
`--timeout`, an optional `--budget-gbp`, and a fresh ephemeral `--state-root`
per attempt. The fresh state root keeps the gate stateless despite Dakar's
reviewed-head cache. Dakar's JSON verdict maps onto the existing host-review
contract: pass or skip is clean, `changes-requested` supplies findings,
`deferred` reuses the established backoff path, and other failures fail the
review. Severity mapping preserves the blocking rule: Dakar `critical` and
...
host-run review, agents never wait on the review tool — the host absorbs
rate-limit backoff in its own wall-clock — and with the default per-work-item
build each builder turn covers one work item, so adapter timeouts only need to
cover honest stage work: roughly 3600 seconds for the build adapter and
4500–5400 seconds for planning and review judgement. Only the legacy
`coderabbitHostReview=false` flow asks implementation agents to wait through
rate-limit backoff with `vsleep` (three retries of up to 90 minutes each), and
only then does the timeout need to reach 21600 seconds.
