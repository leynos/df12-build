...
| Code review                   | Review schemas require verdict and blocker fields; blocker arrays are checked regardless of verdict. Each round runs deterministic gates, then the selected host reviewer when enabled, then reviewer agents. Dakar is always host-run; CodeRabbit host review requires `coderabbitHostReview`, while `false` retains legacy agent-run mode. Both adapters normalize into `HostReviewResult`, share retry policy and severities, and emit bounded telemetry. Between-item review fails closed; terminal deferral halts for assessment.                                                                                                                                                  | Review prompts define what reviewers inspect; host-run modes do not ask agents to invoke the selected reviewer. Legacy CodeRabbit mode retains its agent-run command.                                 | Dakar requires `dakar-review` and `pi` plus `OPENAI_API_KEY`; opt-in CodeRabbit host mode requires CLI auth. Auth failure halts as `fatal-auth`.                                                                       |
