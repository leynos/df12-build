...
| Integration | A JavaScript merge lock serializes integration, and success requires `ok`, `pushed`, `squashMerged`, and roadmap evidence. Addenda whose implementation evidence is complete but whose `ok` field is false become `manual-merge-ready`, never auto-merged. | The integration agent is told to rebase, gate, then squash-merge onto a disposable `integrate-<id>` branch cut from `origin/<base>` inside the task worktree and push it with `git push origin HEAD:<base>`, never switching the root worktree to `<base>`. | GitHub branch permissions, non-fast-forward push handling, and sandbox write access to the task worktree are decisive. |
