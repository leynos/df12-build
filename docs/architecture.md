...
| Salvage | After assessment classifies a failed/halted branch as `continue-manual` or `adopt-partial`, the host may commit verified task-scoped ExecPlan/review artefacts on that same failed task branch with a `df12 salvage v1` subject. It never mutates base, origin, `processed`, or roadmap state. | Assessment and cleanup prompts tell agents to preserve salvage artefacts and recognise the salvage commit convention. | `salvageArtefacts=false` disables the host commit. Verification rejects paths outside the worktree, symlinks, and non-regular files before `git add`. |
...
- Recovery controls: `resumePartialBranches` (opt-in fresh-run discovery),
  ...
  `resumeMaxCandidates`, `salvageArtefacts` (branch-local preservation of
  task-scoped ExecPlan/review artefacts after `continue-manual` or
  `adopt-partial` assessment), and `worktreeWritePreflight` (the host-verified
