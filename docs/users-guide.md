...
before relaunching. Provider faults — rate limits (`429`/`529`), an overloaded
model, or a gateway timeout — are likewise retried in place within the same
`stageAttempts` budget, but each re-run is preceded by a bounded backoff: an
advertised `retry-after` is honoured when present, otherwise a deterministic
seeded jitter within `infraRetryBackoffSeconds` is used. A persistent limit
terminates the task as `provider-fault`. The run result's `faultMetrics` object
counts retries and terminal faults per class (`infraRetries`, `providerRetries`,
`infraFaults`, `providerFaults`, `authFaults`).
