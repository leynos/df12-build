...
hold-only discovery pass still runs before falling through to normal selection
(see Recovery candidate discovery, below); it never advances past discovery
into assessment or review.
...
hold-only pass, regardless of `resumePartialBranches`. Even when it is `false`,
`discoverHeldBranches` reruns steps 1-8 above in full — fetch, canonical
roadmap read, branch listing, id mapping, completed-task filtering, commit
verification, and worktree probing — and feeds the result through the pure
`computeHeldFromDiscovery` to determine which roadmap ids to hold out of normal
selection. This hold-only pass never assesses, resumes, or spawns an agent, and
it skips only step 9 (the `resumeTaskId` and `resumeMaxCandidates` narrowing),
so every surviving branch is held rather than silently dropped by a scoped
discovery that would let non-matching survivors collide on worktree creation.
