...
- Keep the dry-run short-circuit first in `runTask`. The guard is the first
  ...
  guard must remain the first statement in `runTask` to preserve the
  no-mutation guarantee. This boundary is documented consistently in
  `docs/architecture.md`'s `## Enforcement boundary` table (the "Dry run" row)
  and `docs/users-guide.md`'s `## Workflow arguments` section (`dryRun`).
